...
이번에는 계산기의 C 버튼을 클릭했을 때 모든 상태를 초기화하는 초직을 구현해보자. 현재 C 버튼에는 handleClear() 함수가 이벤트 핸들러로 등록되어있으므로 이 함수 내부에 초깃값으로 되돌리는 코드를 추가하면 된다.
...
계산기에서 숫자 버튼 다음에 소수점 버튼을 클릭하면 현재 표시된 숫자 뒤에 소수점이 정상적으로 추가되는 것을 확인할 수 있다. 이미 소수점이 포함된 숫자 (includes('.')) 라면 중복 입력되지 않는다.

---

## 예외 처리하기

계산기를 사용할 때 예기치 않게 발생할 수 있는 오류를 예방하기 위한 예외 처리를 구현해 보자.

현재까지 작성한 계산기 코드에서 오류가 발생하는 대표적인 경우는 아래 두 가지다.

- 숫자를 입력하지 않고 처음부터 연산 기호를 클릭한 경우
- 숫자를 한 번 입력하고 바로 등호를 클릭한 경우

위 두 경우 모두 연산 처리 로직과 관련되어 있으므로 handleOperatorClick() 함수 내부에 예외 처리를 추가해야 한다.
...
export default function App() {
  ...
  const handleOperatorClick = (
    event : React.MouseEvent<HTMLInputElement, MouseEvent>
      ...
      if(state.currentNumber === '0') return; --- (1)
      ...
      } else if (state.currentNumber !== '' && operator === '='){ --- (2)
        setState({
          ...state,
          isNewNumber : true,
        });
...
1. currentNumber 속성의 값이 0 일 때 연산 기호를 클릭하면 return 문을 반환해해 함수 실행을 즉시 종료한다. 이렇게 하면 숫자를 입력하지 않고 연산을 시도하는 잘못된 동작을 방지한다.

2. 숫자를 한 번만 입력한 상태에서 등호 버튼을 클릭한 경우에는 연산은 필요하지 않다. 이때 상태를 유지하면서 isNewNumber만 true로 설정해 다음 입력을 새 숫자로 간주할 수 있도록 한다.

코드를 저장하고 웹 브라우저를 새로 고침한 뒤 예외 상황 두 가지를 테스트해보자. 오류 없이 작동하는 것을 확인할 수 있다.

그런데 소수점이 있는 숫자를 계산하면, 예를 들어 0.1 + 0.2 를 계산하면 0.30000000000000004 와 같이 예상과 다른 결과가 나온다.

![alt text](image-20.png)

이는 자바스크립트가 모든 숫자를 IEEE 754 표준의 64비트 부동소수점 방식으로 처리하기 때문이다. 이 방식은 10진수를 2진 부동소수점으로 반환할 때 무한 반복되는 소수부를 52비트 유효 숫자로 잘라 저장하면서 반올림 오차가 발생한다. 이 오차가 연산 과정에서 누적되어 결과가 그렇게 나오는 것이다. 따라서 예외 처리를 해 사용자 입력으로인한 예기치 않은 동작을 안정적으로 방지해야 한다.

예외 처리를 위해 아래와 같이 추가 패키지를 설치한다.

> TERMINAL\
> npm install decimal.js

패키지 설치가 끝나면 handleOperatorClick() 함수를 아래와 같이 수정한다.

```javascript
import { useState } from "react";
import Decimal from "decimal.js";     --- 추가
    ...
    event : React.MouseEvent<HTMLInputElement, MouseEvent>
      ...
      if(state.previousNumber !== '' && state.operation) {
        ...
        switch (state.operation){
          case '+' :
            // result = prev + current;
            result = new Decimal(prev).plus(current).toNumber(); --- 수정
            break;
          case '-' :
            // result = prev - current;
            result = new Decimal(prev).minus(current).toNumber(); --- 수정
            break;
          case '*' :
            // result = prev * current;
            result = new Decimal(prev).times(current).toNumber(); --- 수정
            break;
          case '/' :
            // result = prev / current;
            result = new Decimal(prev).dividedBy(current).toNumber(); --- 수정
            break;
        }
  };
