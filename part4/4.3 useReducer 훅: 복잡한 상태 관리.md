...
이러한 경우 상태별로 리듀서 함수를 각각 정의하며, 각 상태 변수, 액션 발생 함수, 리듀서 함수의 이름은 중복되지 않아야 한다.

아래는 useReducer 훅을 사용해 3개의 독립적인 상태 (count, use, cart) 를 관리하는 예제다.
...
export default function App(){
    // 카운터 상태 관리
    const [count, countDispatch] = useReducer(counterReducer, 0); --- 1
    // 사용자 상태 관리
    const [user, userDispatch] = useReducer(userReducer, {}); --- 2
    // 장바구니 상태 관리
    const [cart, cartDispatch] = useReducer(cartReducer, []); --- 3
...
1. count 상태 관리 : counterReducer() 함수를 사용해 숫자 상태를 관리한다. 버튼 클릭 시 'INCREMENT' 액션이 발생하며, 숫자가 1씩 증가한다.

2. user 상태 관리 : userReducer() 함수를 사용해 사용자 정보를 관리한다. 'SET_USER' 액션이 발생하면 user.name 값을 'Alice'로 설정한다.

3. cart 상태 관리 : cartReducer() 함수를 사용해 장바구니 배열을 관리한다. 'ADD_ITEM' 액션이 발생하면 { id: 1, name: 'Item 1' } 이 배열에 추가된다.
