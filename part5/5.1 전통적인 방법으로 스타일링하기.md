...
3. btn 클래스는 항상 적용되고, isActive가 true 일 때만 is-active 클래스가 함께 추가된다. 이 경우 \<button> 요소의 클래스는 btn is-active 가 된다.

실행한 후 개발자 도구를 열어 \<button> 요소를 확인해보면 조합된 클래스 이름이 적용된 것을 볼 수 있다.
...
이번에는 CSS 모듈과 함께 classNames를 사용하는 방법을 알아보자.

CSS 모듈에서 classNames 라이브러리를 사용할 때는 classnames/bind 패키지를 활용한다. 해당 패키지가 제공하는 bind() 함수를 사용해 CSS 모듈 객체를 classNames 함수에 전달한다. 여기서 **CSS 모듈 객체** 란 .module.css 확장자를 가진 CSS 파일을 컴포넌트에서 import할 때 생성되는 객체를 의미한다. 이 객체에서는 정의된 클래스 이름이 키로 고유한 해시가 적용된 클래스명이 값으로 들어 있다. 따라서 bind(객체명) 과 같이 사용하면 객체 접근 없이 문자열 키만으로 클래스 이름을 동적으로 조합할 수 있다.
...
export default function App(){
  ...
  return (
    <>
      <button className={cx({ btn: true, 'is-active': isActive })}> --- 3
        App Button</button>
      <Child />
    </>
  );
...
1. classnames/bind 패키지에서 classNames 함수를 import 한다.

2. bind() 함수를 사용해 CSS 모듈 객체(styles)와 연결한다. 이렇게 하면 객체 속성에 접근하지 않고도 cx('클래스명') 처럼 간단한 문자열로 스타일을 사용할 수 있다.

3. btn이 true이므로 btn 클래스는 항상 적용되고, is-active 클래스는 isActive 가 true 일 때만 함께 추가된다.

![alt text](image-7.png)

![alt text](image-8.png)

이처럼 CSS 모듈에서는 같은 classNames 함수를 사용하더라도 컴포넌트마다 클래스 이름이 고유하게 변환되기 때문에 스타일 충돌을 걱정하지 않아도 된다.
